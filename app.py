...
import openpyxl
from gst_automation_playwright import run_batch_gst_search_excel, get_captcha_state, submit_captcha_solution
...
def upload_file():
    ...
    threading.Thread(target=process_excel_background, args=(job_id, input_path, output_filename), daemon=True).start()
...
@app.route('/captcha')
def get_captcha():
    """Returns the current captcha image (if waiting) for the web UI to display."""
...
def solve_captcha():
    """Receives the user's captcha solution from the web UI."""
    data = request.get_json()
    solution = data.get("solution", "").strip()
    if solution:
        submit_captcha_solution(solution)
        return jsonify({"status": "ok"})
    return jsonify({"error": "Empty solution"}), 400
